var queryString, svConfigs, agentUrl, visitorUrl, ms = Date.now(), currVersion = ms,
    lsRepUrl = window.location.protocol + "//" + window.location.host + "/", autoReconnectInterval = 5000,
...
$(function () {
    var a = function (a, b) {
        ...
        if (queryString.isAdmin) {
            var e = queryString.p === void 0 ? null : JSON.parse(decodeURIComponent(escape(window.atob(queryString.p))));
            agentUrl = lsRepUrl + "pages/r.html?room=" + queryString.room + "&p=" + queryString.p + "&isAdmin=1", delete e.isAdmin, e = window.btoa(unescape(encodeURIComponent(JSON.stringify(e)))), e = e.split("=").join(""), visitorUrl = lsRepUrl + "pages/r.html?room=" + queryString.room + "&p=" + e
        }
    }, b = function () {
        ...
        a(c), $.get(lsRepUrl + "pages/version.txt?v=" + ms, function (a) {
            currVersion = a;
            var c = function () {
                if (!svConfigs.iceServers.turnon) $.ajax({
                    type: "POST",
                    url: lsRepUrl + "server/script.php",
                    data: {type: "getpk"}
                }).done(function (a) {
                    if (a) {
                        svConfigs.iceServers.turnon = a;
                        var c = {lsRepUrl: lsRepUrl, lang: svConfigs.smartVideoLanguage};
                        smartVideoLocale.init(c, jQuery), b()
                    }
                }).fail(function () {
                }); else {
